"""LookaheadCriticBot: same contract as LookaheadBot (docs/lookahead_bot_plan.md) —
must return legal actions, never mutate the live env while searching, and play a
full episode through cleanly — but driven by beam search + a trained Critic
instead of alpha-beta + a hand-crafted heuristic.
"""
...
def _tiny_bot(**kwargs):
    # Small time budget + beam width so the test suite stays fast.
    return LookaheadCriticBot(time_budget=0.05, beam_width=3, **kwargs)


def test_lookahead_critic_bot_does_not_mutate_the_live_env():
    env = WarChestEnv()
    env.reset()
    bot = _tiny_bot()
...
def test_lookahead_critic_bot_plays_a_full_legal_episode():
    env = WarChestEnv()
    env.reset()
    bot = _tiny_bot()
    for _ in range(30):
...
def test_lookahead_critic_bot_fair_mode_plays_a_full_legal_episode():
