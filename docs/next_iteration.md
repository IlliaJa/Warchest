...
| Reward-table tuning | **not the constraint**; hygiene **done 2026-08-09** | both listed items shipped with `IDEAS.md` L8: `ATTACK_REWARD` 0.02 → **0.0** (it double-paid what material PBRS covers, flagged since 2026-07-03), and `holding_reward_rate` re-derived on `TYPICAL_MAIN_TURNS = 37` rather than the assumed 150 — **0.001067 → 0.004324, 4.05×**, so it is now the mechanism it was designed to be. L8's own item shipped alongside: the per-move step penalty became a per-*turn* tempo cost, which stops tactic continuations, Berserker chains and the Swordsman bonus move paying it once per maneuver. Standing caveat: all three change the reward scale, so `score` / returns / `critic_mae` do **not** compare across 2026-08-09; win rate and the gauntlet do |
